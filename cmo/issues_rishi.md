...
- [x] (2026-04-02) HQ autopush flock errors (`flock: failed to execute -w` / `flock: -c requires exactly one command argument`) — Fixed by updating HQ autopush crons to call `python3 cmo/scripts/hq_autopush_locked.py` (fcntl lock; no `flock`), and deprecating the legacy `cmo/scripts/run_autopush.py`.
